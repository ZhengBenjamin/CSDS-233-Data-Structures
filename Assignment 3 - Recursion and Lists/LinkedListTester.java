...
public class LinkedListTester {
    ...
    private LinkedList emptyList; //Empty list 
    private LinkedList largeList; //Predefined list with large number of elements
    private LinkedList repeatingList; //List with repeating elements
    private LinkedList repeatingList2; //List with different repeating elements 
    
    @Before 
    public void setup() {
        emptyList = new LinkedList();
        largeList = new LinkedList(); 
        repeatingList = new LinkedList();
        repeatingList2 = new LinkedList();
    ...
    @Test 
    public void testAdd() {
        for (int i = 0; i <= 10000; i++) { //Stress test with 10000 elements
            emptyList.add(i);
    ...
    public void testSublist() {
        LinkedList list = largeList.sublist(0, 1000); //Sublist of large list
    ...
    public void removeNoise() {
        LinkedList list = largeList.removeNoise(); //Removes noise from large list
        ...
        LinkedList list2 = repeatingList.removeNoise(); //Removes noise from repeating list
        assertEquals(list2.get(0), 12);
        assertEquals(list2.get(list.getSize() - 1), 12);

        LinkedList list3 = emptyList.removeNoise(); //Empty list
        assertEquals(list3.getSize(), 0); //Check if list is empty

        LinkedList list4 = repeatingList2.removeNoise(); //Removes noise from repeating list 2
